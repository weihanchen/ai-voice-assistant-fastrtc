...
- **FR-011**: 系統必須支援角色專屬流程模式配置 (preferred_flow_mode)，允許不同角色使用不同的處理流程
- **FR-012**: 面試官和教練角色必須使用 "tools" 流程模式（對話型），助理角色必須使用 "multi_agent" 流程模式（任務型）
- **FR-013**: 角色的 preferred_flow_mode 設定必須覆蓋全域 FLOW_MODE 環境變數
